...
describe.only('svg-baker-runtime/browser-sprite', () => {
  describe('constructor()', () => {
    it('should assign default config properly', () => {
      const sprite = new Sprite({ qwe: 123 });

      sprite.config.should.have.all.keys([
        // super class config
        'attrs',

        // default options
        'mountTo',
        'locationUpdateEventName',
        'referencesToUpdate',
        'baseFix',
        'angularBaseFix',
    ...
    it('should subscribe to location updated event and call `updateUrls` when it occurs', () => {
      ...
      sprite.mount();
      dispatchCustomEvent(sprite.config.locationUpdateEventName, {});
  ...
  describe('updateUrls()', () => {
    ...
    it('should update urls in sprite symbols and in references', () => {
      ...
      const ref = parse(wrapInSvg('<use xlink:href="#twitter"></use>'));
